...
let declare_unsynchronized_option name v =
  check_key name;
  declare_raw name v
...
let declare_option ?(preprocess = fun x -> x)
  { optkind=kind; optstage=stage; optdepr=depr; optkey=key; optread=read; optwrite=write } =
  check_key key;
  let default = read() in
...
let newsig kind (oldsig:_ OldSig.t) = {
  optkind = kind;
  optstage = oldsig.optstage;
  optdepr = oldsig.optdepr;
  optkey = oldsig.optkey;
  optread = oldsig.optread;
  optwrite = oldsig.optwrite;
}

let declare_int_option ?preprocess oldsig =
  declare_option ?preprocess (newsig IntKind oldsig)

let declare_bool_option ?preprocess oldsig =
  declare_option ?preprocess (newsig BoolKind oldsig)

let declare_string_option ?preprocess oldsig =
  declare_option ?preprocess (newsig StringKind oldsig)

let declare_stringopt_option ?preprocess oldsig =
  declare_option ?preprocess (newsig StringOptKind oldsig)

...
Require Ltac2.Ind Ltac2.Array Ltac2.Env.

Ltac2 @external type_in : env -> constr -> constr := "rocq-runtime.plugins.ltac2" "constr_type_in".

Ltac2 type (c : constr) : constr := type_in (Env.current_env()) c.
...
Ltac2 @external make_in : env -> ident option -> constr -> binder := "rocq-runtime.plugins.ltac2" "constr_binder_make_in".
(** Create a binder given the name and the type of the bound variable.
    Fails if the type is not a type in the given env. *)

Ltac2 make (na:ident option) (t : constr) : binder := make_in (Env.current_env()) na t.
(** Create a binder given the name and the type of the bound variable.
...
Ltac2 @external check_in : env -> constr -> constr result := "rocq-runtime.plugins.ltac2" "constr_check_in".
(** Checks that a constr (typically generated by unsafe means) is indeed safe in the
    given environment, and returns it, or the error otherwise. *)

Ltac2 check (c : constr) : constr result := check_in (Env.current_env()) c.
...
Ltac2 @external subst_vars : ident list -> constr -> constr := "rocq-runtime.plugins.ltac2" "constr_subst_vars".
...
Ltac2 @external in_context_in : env -> ident -> constr -> (unit -> unit) -> constr := "rocq-runtime.plugins.ltac2" "constr_in_context_in".
(** [in_context_in Γ id c tac] evaluates [tac] in a fresh focused goal
    [Γ, id : c ⊢ ?X] (where [?X] is a fresh type evar in [Γ, id : c])
    and returns [λ (id : c) => t] where [t] is the proof built by the tactic. *)

Ltac2 in_context (id : ident) (c : constr) (tac : unit -> unit) : constr :=
  in_context_in (Env.goal_env()) id c tac.
...
Module Pretype.
  ...
  Ltac2 @external pretype_in : Flags.t -> env -> expected_type -> preterm -> constr
    := "rocq-runtime.plugins.ltac2" "constr_pretype_in".
  (** Pretype the provided preterm in the given environment. *)

  Ltac2 pretype (flags : Flags.t) (t : expected_type) (c : preterm) :=
    pretype_in flags (Env.current_env()) t c.
  (** Pretype the provided preterm. Assumes the goal to be focussed. *)
End Pretype.

Ltac2 pretype_in (env : env) (c : preterm) : constr :=
  Pretype.pretype_in Pretype.Flags.constr_flags env Pretype.expected_without_type_constraint c.
(** Pretype the provided preterm in the provided environment. *)

Ltac2 pretype (c : preterm) : constr := pretype_in (Env.current_env()) c.
(** Pretype the provided preterm. Requires 0 or 1 goal under focus. *)

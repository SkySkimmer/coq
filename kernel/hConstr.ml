...
(* local_env: can't use actual local env unless we start taking
   Environ.env to deal with Case
   instead use physical equality and make fresh int refs

   XXX this is terrible
   instead generate a unique id for each binder crossed
   and have a field in hconstr containing the id for each rel
   the field participates to both hash and equality
   the rel is still there so liftings are still handled

   or even generate unique id only for the case binders where we don't have the type
*)
